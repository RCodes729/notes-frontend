...
export default function LoginPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setErr(Array.isArray(msg) ? msg.join(', ') : msg || 'Login failed');
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="min-h-screen flex items-center justify-center p-6 bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950">
      <form onSubmit={onSubmit} className="w-full max-w-md bg-white/10 backdrop-blur-lg border border-white/20 rounded-2xl p-6 space-y-4 shadow-2xl">
        ...
        <button disabled={loading} className="w-full rounded-xl py-2 bg-indigo-600 hover:bg-indigo-500 text-white">
          {loading ? 'Signing in...' : 'Login'}
        </button>

        <p className="text-sm text-slate-300">
          No account? <Link className="text-indigo-300 underline" href="/signup">Sign up</Link>
        </p>
      </form>
    </main>
  );
